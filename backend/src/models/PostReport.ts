...
export interface IPostReport {
  id: string;
  postId: string; // ID của bài viết bị báo cáo
  reporterId: string; // ID người báo cáo
  reportedUserId: string; // ID người bị báo cáo (author của post)
  reason: string; // Lý do báo cáo
  description: string; // Mô tả chi tiết
  postContent: string; // Nội dung bài viết bị báo cáo (caption)
  postMedia?: Array<{
    type: "image" | "video";
    sourceUrl: string;
  }>; // Media của bài viết
...
export class PostReport {
  private static collection = "post_reports";

  static async create(
    reportData: Omit<IPostReport, "id" | "createdAt" | "updatedAt">
  ): Promise<IPostReport> {
    if (!firestore) {
  ...
  static async findAll(status?: string): Promise<IPostReport[]> {
    if (!firestore) {
      throw new Error("Firestore not initialized");
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[PostReport.findAll] orderBy failed, continuing without order:",
          orderError.message
        );
      }
      ...
      console.error("[PostReport.findAll] Error:", {
      ...
      if (error.code === 9 || error.message?.includes("index")) {
        throw new Error(
          'Firestore index chưa được tạo. Vui lòng tạo composite index cho collection "post_reports" với fields: status (Ascending), createdAt (Descending). ' +
  ...
  static async findById(id: string): Promise<IPostReport | null> {
    if (!firestore) {
      throw new Error("Firestore not initialized");
    }
