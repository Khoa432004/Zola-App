...
export default function UserManagement() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        setError(err.message || "Lỗi khi tải danh sách user");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleBanUser = async (userId: string) => {
    ...
    if (
      !confirm(
        `Bạn có chắc muốn ban tài khoản "${user?.name}"?\n\nUser này sẽ không thể đăng nhập lại.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        setUsers(
          users.map((u) =>
            u.id === userId
              ? { ...u, status: "banned" as const, isDisabled: true }
              : u
          )
        );
        ...
        alert(response.message || "Đã ban tài khoản user");
      } else {
        throw new Error(response.message || "Không thể ban user");
      }
    } catch (err: any) {
      ...
      alert("Lỗi: " + (err.message || "Không thể ban user"));
    }
  };
    ...
    try {
      ...
      if (response.success) {
        ...
        setUsers(
          users.map((u) =>
            u.id === userId
              ? { ...u, status: "active" as const, isDisabled: false }
              : u
          )
        );
        ...
        alert(response.message || "Đã mở khóa tài khoản user");
      } else {
        throw new Error(response.message || "Không thể mở khóa user");
      }
    } catch (err: any) {
      ...
      alert("Lỗi: " + (err.message || "Không thể mở khóa user"));
    }
  };
  ...
  const handleChangeRole = (userId: string, newRole: "user" | "admin") => {
    ...
    if (
      confirm(
        `Bạn có chắc muốn đổi role của "${user?.name}" thành ${roleLabel}?`
      )
    ) {
      setUsers(
              ...
              {["all", "active", "banned"].map((status) => (
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    backgroundColor:
                      filterStatus === status ? "#2563eb" : "#f3f4f6",
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    backgroundColor:
                      filterRole === role.value ? "#7c3aed" : "#f3f4f6",
            ...
            filteredUsers.map((user) => (
              <div
                ...
                style={{
                  ...
                  backgroundColor:
                    selectedUser?.id === user.id ? "#eff6ff" : "#ffffff",
                  transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontWeight: "bold",
                        backgroundColor:
                          user.status === "banned" ? "#9ca3af" : undefined,
                        background:
                          user.status === "active"
                ...
                >
                  ...
                  <p
                    style={{
                      fontWeight: "600",
                      color:
                        selectedUser.reportCount > 5 ? "#dc2626" : "#111827",
                      margin: 0,
                    }}
                  >
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontWeight: "600" }}>
                      Cảnh báo: User này có nhiều báo cáo vi phạm!
                    </span>
                  </div>
                </div>
              )}
                  ...
                  {selectedUser.status === "active" ? (
                    <button
                      ...
                      style={{
                        ...
                        padding: "14px 24px",
                        backgroundColor:
                          selectedUser.role === "admin" ? "#9ca3af" : "#dc2626",
                        color: "#ffffff",
                        border: "none",
                        borderRadius: "8px",
                        fontWeight: "600",
                        ...
                        cursor:
                          selectedUser.role === "admin"
                            ...
                            : "pointer",
                        transition: "all 0.2s",
                        opacity: selectedUser.role === "admin" ? 0.6 : 1,
                      }}
                      onMouseEnter={(e) => {
